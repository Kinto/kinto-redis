...
class Storage(MemoryBasedStorage):
    ...
    *(Optional)* Instance location URI can be customized::
    ...
    @wrap_redis_error
    def bump_and_store_timestamp(self, resource_name, parent_id, obj=None,
    ...
    @wrap_redis_error
    def create(self, resource_name, parent_id, obj, id_generator=None,
        ...
        with self._client.pipeline() as multi:
            ...
            multi.srem(
                '{0}.{1}.deleted'.format(resource_name, parent_id),
                _id
            )
            multi.execute()
    ...
    @deprecate_kwargs({"collection_id": "resource_name", "record": "obj"})
    @wrap_redis_error
    def update(self, resource_name, parent_id, object_id, obj,
               id_field=DEFAULT_ID_FIELD,
               modified_field=DEFAULT_MODIFIED_FIELD,
               auth=None):
        obj = ujson.loads(self.json.dumps(obj))
    ...
    @deprecate_kwargs({"collection_id": "resource_name"})
    @wrap_redis_error
    def delete(self, resource_name, parent_id, object_id,
        ...
        self.set_object_timestamp(resource_name, parent_id, existing,
                                  modified_field=modified_field,
                                  last_modified=last_modified)
        existing = self.strip_deleted_object(resource_name, parent_id,
                                             existing)

        if with_deleted:
            deleted_obj_key = '{0}.{1}.{2}.deleted'.format(resource_name,
                                                              parent_id,
                                                              object_id)
            with self._client.pipeline() as multi:
                multi.set(
    ...
    @deprecate_kwargs({"collection_id": "resource_name"})
    @wrap_redis_error
    def purge_deleted(self, resource_name, parent_id, before=None,
                      id_field=DEFAULT_ID_FIELD,
                      modified_field=DEFAULT_MODIFIED_FIELD,
                      auth=None):
        ...
        for i, ids in enumerate(results):
            ...
            keys = ['{0}.{1}.{2}.deleted'.format(resource_name, parent_id,
                                                 _id.decode('utf-8'))
                    for _id in ids]
            ...
            if len(to_remove) > 0:
                with self._client.pipeline() as pipe:
                    pipe.delete(*['{0}.{1}.{2}.deleted'.format(
                        resource_name, parent_id, _id) for _id in to_remove])
    ...
    def _get_objects_by_parent_id(self, parent_id, resource_name, with_meta=False):
        ...
        for i, ids in enumerate(results):
            ...
            if with_meta:
                resource_objects = [dict(__resource_name__=resource_name,
    ...
    def list_all(self, resource_name, parent_id, filters=None, sorting=None,
        ...
        if include_deleted:
            ...
            for i, ids in enumerate(results):
                ...
                deleted_keys = ['{0}.{1}.{2}.deleted'.format(resource_name,
                                                             parent_id,
                                                             _id.decode('utf-8'))
                                for _id in ids]
        ...
        objects, _ = self.extract_object_set(objects + deleted,
                                             filters, sorting,
    ...
    @wrap_redis_error
    def count_all(self, resource_name, parent_id, filters=None, sorting=None,
                  pagination_rules=None, limit=None, include_deleted=False,
                  id_field=DEFAULT_ID_FIELD,
                  modified_field=DEFAULT_MODIFIED_FIELD,
                  deleted_field=DEFAULT_DELETED_FIELD,
        ...
        _, count = self.extract_object_set(objects,
                                           filters, sorting,
                                           id_field, deleted_field,
                                           pagination_rules, limit)
    ...
    @deprecate_kwargs({"collection_id": "resource_name"})
    @wrap_redis_error
    def delete_all(self, resource_name, parent_id, filters=None,
                   sorting=None, pagination_rules=None, limit=None,
                   id_field=DEFAULT_ID_FIELD, with_deleted=True,
                   modified_field=DEFAULT_MODIFIED_FIELD,
                   deleted_field=DEFAULT_DELETED_FIELD,
                   auth=None):
        objects = self._get_objects_by_parent_id(parent_id, resource_name, with_meta=True)

        objects, count = self.extract_object_set(objects=objects,
                                                 filters=filters,
                                                 sorting=sorting,
        ...
        deleted = [self.delete(r.pop('__resource_name__'),
                               r.pop('__parent_id__'),
                               r[id_field],
